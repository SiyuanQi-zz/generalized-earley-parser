...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch > 30:
            epoch_error = validate_baseline(test_loader, model, args=args)
        else:
            epoch_error = 1.0
...
def inference(model_outputs, activities, sequence_ids, ctc_labels, args):
    ...
    for batch_i in range(model_outputs.size()[1]):
        grammar_file = os.path.join(args.tmp_root, 'grammar', 'wnp', activities[batch_i]+'.pcfg')
        grammar = parser.grammarutils.read_grammar(grammar_file, index=True, mapping=datasets.wnp_metadata.subactivity_index)
...
def predict(activities, total_lengths, labels, ctc_labels, batch_tokens, batch_seg_pos):
    ...
    with open(os.path.join(args.tmp_root, 'prior', 'wnp', 'duration_prior.json')) as f:
    ...
    for batch_i in range(np_labels.shape[1]):
        ...
        for frame in range(total_lengths[batch_i]-duration):
            ...
            duration_prediction = [current_token for _ in range(min(duration, max(10, sample_duration-(frame-seg_start_frame))))]
...
def validate(val_loader, model, args, test=False):
    ...
    for i, (features, labels, probs, total_lengths, ctc_labels, ctc_lengths, activities, sequence_ids) in enumerate(val_loader):
        # if sequence_ids[0] != 'data_01-21-56':
        #     continue
        features = utils.to_variable(features, args.cuda)
        labels = utils.to_variable(labels, args.cuda)
        ...
        for batch_i in range(labels.size()[1]):
            aog_result = np.load(os.path.join(args.tmp_root, 'iccv_results', activities[batch_i], '{}.npy'.format(sequence_ids[batch_i])))
            # aog_result = np.hstack((aog_result, aog_result[-1]))
            aog_result = np.hstack((aog_result[0], aog_result))
            vizutil.plot_segmentation(
                [labels[:, batch_i].squeeze(), aog_result, pred_labels[:, batch_i].squeeze(), batch_earley_pred_labels[batch_i]],
                int(total_lengths[batch_i]), filename=os.path.join(args.tmp_root, 'visualize', 'segmentation', 'wnp', '{}_{}.png'.format(activities[batch_i], sequence_ids[batch_i])), border=False, vmax=len(datasets.wnp_metadata.subactivities))
            # vizutil.plot_segmentation(
    ...
    confusion_matrix = sklearn.metrics.confusion_matrix(all_gt_detections, all_detections, labels=range(len(datasets.wnp_metadata.subactivities)))
    vizutil.plot_confusion_matrix(confusion_matrix, datasets.wnp_metadata.subactivities[:], normalize=True, title='', filename=os.path.join(args.tmp_root, 'visualize', 'confusion', 'wnp', 'detection.pdf'))
    confusion_matrix = sklearn.metrics.confusion_matrix(all_gt_frame_predictions, all_frame_predictions, labels=range(len(datasets.wnp_metadata.subactivities)))
    vizutil.plot_confusion_matrix(confusion_matrix, datasets.wnp_metadata.subactivities[:], normalize=True, title='', filename=os.path.join(args.tmp_root, 'visualize', 'confusion', 'wnp', 'prediction_frame.pdf'))
    confusion_matrix = sklearn.metrics.confusion_matrix(all_gt_seg_predictions, all_seg_predictions, labels=range(len(datasets.wnp_metadata.subactivities)))
    vizutil.plot_confusion_matrix(confusion_matrix, datasets.wnp_metadata.subactivities[:], normalize=True, title='', filename=os.path.join(args.tmp_root, 'visualize', 'confusion', 'wnp', 'prediction_seg.pdf'))
...
def validate_baseline(val_loader, model, args, test=False):
    ...
    for i, (features, labels, probs, total_lengths, ctc_labels, ctc_lengths, activities, sequence_ids) in enumerate(val_loader):
        ...
        for batch_i in range(model_outputs.size()[1]):
            model_probs = torch.nn.Softmax(dim=1)(model_outputs[:int(total_lengths[batch_i]), batch_i, :].squeeze()).cpu().data.numpy()
            np.save(os.path.join(args.tmp_root, 'nn_output', activities[batch_i], sequence_ids[batch_i]), model_probs)
...
def parse_arguments():
    ...
    parser.add_argument('--log-root', default=os.path.join(paths.log_root, 'wnp/parsing', default_setting), help='log files path')
    parser.add_argument('--resume', default=os.path.join(paths.tmp_root, 'checkpoints/wnp/parsing', default_setting), help='path to latest checkpoint')
